...
public class ManagerDB {
    ...
    public Manager ManagerLogIn(string email, string password) {
       using SqlConnection connection = new SqlConnection(Secrets.getConnectionString());
        ...
        try {
            using SqlCommand command = new SqlCommand("SELECT top 1 * FROM Managers WHERE Email = @Email AND managerPW = @Password", connection);
            command.Parameters.AddWithValue("@Email", email);
            ...
            if(reader.HasRows) {
                ...
                List<Employee> emplList = emplDB.GetAssociatedEmployees(reader.GetString(0));
                foreach(Employee empl in emplList) {
                    empl.ListOfExpenses = ticketDB.GetExpenseForEmpl(empl.email);
                }
                return new Manager(reader.GetString(1), reader.GetString(2), reader.GetString(0), emplList);
            }
            return null;
        } catch (Exception e) {
            Console.WriteLine(e);
